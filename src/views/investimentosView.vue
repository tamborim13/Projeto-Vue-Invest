    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            unsubscribe = onSnapshot(q, (snapshot) => {
                investimento.value = snapshot.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data()
                }))
    ...
    <div class="view-container">
        <header class="view-header">
            <div class="title-group">
                <h1>Meus <span>investimento</span></h1>
        ...
        <section class="table-card">
            ...
            <table v-else-if="investimento.length" class="custom-table">
                <thead>
                    <tr>
                        <th>DESCRIÇÃO</th>
                        <th>DATA</th>
                ...
                <tr v-for="item in investimento.filter(i => i.tipo === 'Investimento')" :key="item.id">
                    <td>{{ item.descricao }}</td>
                    <td>{{ item.data }}</td>
                    <td class="valor-verde">R$ {{ item.valor.toFixed(2) }}</td>
                    <td class="text-center">
                        <button class="btn-delete" @click="DeletarInvestimento(item.id)" title="Excluir">
            ...
            <div v-if="showModal" class="modal-overlay" @click.self="toggleModal">
                <div class="modal-content">
                    ...
                    <footer class="modal-footer">
                        ...
                        <button class="btn-save" @click="SalvarInvestimento">SALVAR INVESTIMENTO</button>
                    </footer>
                </div>
            </div>
        </Transition>
